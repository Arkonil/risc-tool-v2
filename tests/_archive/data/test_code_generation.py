...
def test_huge_uuid_ids_render_as_compact_hex_placeholders():
    big_group_int = 2**100 + 12345
    big_iter_int = 2**64 + 15
    group_suffix = f"h{big_group_int:x}"
    iter_suffix = f"h{big_iter_int:x}"

    iteration = NumericalSingleVarIteration(
        uid=IterationID(big_iter_int),
        name="Big ID Iteration",
        variable_name="credit_score",
        risk_segment_details=RiskSegmentConfig(),
    )
    iteration.set_default_groups(
        OrderedDict([
            (
                GroupID(big_group_int),
                NumericalGroup(lower_bound=0.0, upper_bound=100.0),
            ),
        ])
    )

    python_code = _render(
        iteration.generate_python_code(False),
        {
            "DATA": "data",
            "VARIABLE_NAME": "credit_score",
            "OUTPUT_VARIABLE_NAME": "Risk_Seg_big_custom",
            f"GROUP_INDEX_{group_suffix}": '"BIG"',
        },
    )

    assert f"iter_{iter_suffix}_map = {{" in python_code
    assert "pd.Interval(0.0, 100.0, closed='right'): \"BIG\"," in python_code
    assert "$GROUP_INDEX_" not in python_code

    sas_code = _render(
        iteration.generate_sas_code(False),
        {
            "VARIABLE_NAME": "credit_score",
    ...
    assert 'out = "BIG";' in sas_code
    assert "$GROUP_INDEX_" not in sas_code
...
def test_get_code_templates_single_chain(code_session):
    ...
    assert (
        substitute["OUTPUT_VARIABLE_NAME"]
        == f"Risk_Seg_{short_id(num_iter.uid)}_custom"
    )
...
def test_get_code_templates_double_chain_links_previous_output(code_session):
    ...
    assert templates[1]["substitute"]["OUTPUT_VARIABLE_NAME"] == (
        f"Risk_Seg_{short_id(double_iter.uid)}_custom"
    )
...
def test_get_code_templates_default_flag_names_output(code_session):
    ...
    assert default_templates[0]["substitute"]["OUTPUT_VARIABLE_NAME"] == (
        f"Risk_Seg_{short_id(num_iter.uid)}_default"
    )
...
def test_get_sas_code_with_macro(code_session):
    ...
    assert f"%let result_0_ = Risk_Seg_{short_id(num_iter.uid)}_custom;" in code
...
def test_get_sas_code_without_macro(code_session):
    ...
    assert f"format Risk_Seg_{short_id(num_iter.uid)}_custom $50.;" in code
...
def test_python_code_executes_single_var_numerical(code_session):
    ...
    data = namespace["data"]
    output_col = f"Risk_Seg_{short_id(num_iter.uid)}_custom"
...
def test_python_code_executes_single_var_categorical(code_session):
    ...
    data = namespace["data"]
    output_col = f"Risk_Seg_{short_id(cat_iter.uid)}_custom"
...
def test_python_code_executes_double_var_chain(code_session):
    ...
    data = namespace["data"]
    output_col = f"Risk_Seg_{short_id(double_iter.uid)}_custom"

    assert output_col in data.columns
    assert data[output_col].notna().all()
    # Identity grid: the double variable maps each row to the parent segment.
    assert list(data[output_col]) == list(
        data[f"Risk_Seg_{short_id(num_iter.uid)}_custom"]
    )
...
def test_python_code_executes_with_default_groups(code_session):
    ...
    data = namespace["data"]
    output_col = f"Risk_Seg_{short_id(num_iter.uid)}_default"
    segments = num_iter.risk_segment_details.segments
