...
def _f(query: str, columns: list[str]) -> Filter:
    ...
    """
    f = Filter(uid=FilterID(1), name="test", query=query)
...
def test_filter_compilation_and_execution() -> None:
    ...
    f1 = Filter(uid=FilterID(1), name="f1", query="value == 10")
    ...
    f2 = Filter(uid=FilterID(2), name="f2", query="`Credit Score` >= 700")
    ...
    f3 = Filter(uid=FilterID(3), name="f3", query="100 < value <= 200")
    ...
    f4 = Filter(uid=FilterID(4), name="f4", query="status in ('Approved', 'Pending')")
    ...
    f5 = Filter(uid=FilterID(5), name="f5", query="value.isna()")
    ...
    f6 = Filter(uid=FilterID(6), name="f6", query="~value.isna()")
...
def test_invalid_filter_queries() -> None:
    ...
    with pytest.raises(InvalidFilterError):
        f = Filter(uid=FilterID(1), name="test", query="")
        f.validate_query()

    # Query cannot contain assignments
    with pytest.raises(InvalidFilterError):
        f = Filter(uid=FilterID(1), name="test", query="x = 10")
    ...
    with pytest.raises(InvalidFilterError):
        f = Filter(uid=FilterID(1), name="test", query="missing_col > 5")
...
def test_comprehensive_string_methods() -> None:
    ...
    with pytest.raises(InvalidFilterError):
        f_invalid = Filter(uid=FilterID(1), name="invalid", query="name.str.contains('li')")
