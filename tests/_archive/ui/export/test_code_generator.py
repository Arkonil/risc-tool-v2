...
from risc_tool.data.models.uid import RiskSegmentID, short_id
...
def test_python_code_generation(
    ...
    assert f"iter_{int(iteration.uid)}_map = {{" in code
    assert (
        f'create_mapped_variable(data["credit_score"], iter_{int(iteration.uid)}_map)'
        in code
    )
...
def test_sas_code_generation_without_macro(
    ...
    assert f"Risk_Seg_{short_id(iteration.uid)}_custom" in code
