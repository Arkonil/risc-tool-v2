...
def test_dev_bad_rates_must_share_current_rate_mob() -> None:
    """Dev unit and dollar bad rates must share the same current rate MOB."""
    scalar_config = ScalarConfig()

    mismatched_dev_unit = _dev_unit_br().with_updates(current_rate_mob=18)
    ...
    for cls in (SimulationConfig, SimulationConfigGenerator):
        ...
        try:
            ...
            assert "same current rate MOB" in str(e)


def test_test_bad_rates_must_share_current_rate_mob() -> None:
    """Test unit and dollar bad rates must share the same current rate MOB."""
    scalar_config = ScalarConfig()

    mismatched_test_dollar = _test_dollar_br().with_updates(
        current_rate_mob=24, is_annualized=True
    )
...
def test_bad_rate_mob_equality_skipped_when_side_missing() -> None:
    """MOB equality is only enforced when both unit and dollar sides are present."""
    scalar_config = ScalarConfig()
...
def test_scg_lifetime_mob_default_roundtrip_and_hash() -> None:
    """Lifetime MOB defaults to 36, is carried and serialized, and affects the hash."""
    scalar_config = ScalarConfig()

    def make_scg(mob: int | None = None) -> SimulationConfigGenerator:
        kwargs: dict = {
            "name": "base",
            "risk_segment_config": _risk_config(),
            "dev_unit_bad_rate": _dev_unit_br(),
        ...
        if mob is not None:
            kwargs["lifetime_rate_mob"] = mob
        return SimulationConfigGenerator(**kwargs)

    default_scg = make_scg()
    assert default_scg.lifetime_rate_mob == 36

    updated = default_scg.with_updates(lifetime_rate_mob=48)
    assert updated.lifetime_rate_mob == 48

    restored = SimulationConfigGenerator.from_dict(updated.to_dict())
    assert restored.lifetime_rate_mob == 48

    changed = make_scg(mob=60)
    assert default_scg.uid != changed.uid
