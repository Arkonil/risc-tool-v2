...
def test_filter_compilation_and_execution() -> None:
    ...
    res = lf.filter(f1.filter_expr).collect()
    assert len(res) == 1
    ...
    res = lf.filter(f2.filter_expr).collect()
    assert len(res) == 2
    ...
    res = lf.filter(f3.filter_expr).collect()
    assert len(res) == 1
    ...
    res = lf.filter(f4.filter_expr).collect()
    assert len(res) == 2
...
def test_outlier_rule_thresholds_and_frequency() -> None:
    ...
    filtered_lf = lf.filter(rule.filter_expr).collect()
...
def test_comprehensive_arithmetic_operations() -> None:
    ...
    f3 = _f("(x // y) % 2 == 0", ["x", "y"]) # 10//2=5 (mod 2 = 1), 20//3=6 (mod 2 = 0), 30//4=7 (mod 2 = 1)
...
def test_comprehensive_boolean_operations() -> None:
    ...
    lf = pl.LazyFrame({"id": [1, 2, 3, 4], "x": [True, True, False, False], "y": [True, False, True, False]})
...
@pytest.mark.parametrize("func_name, expr_str, val_list, expected_matches", [
...
def test_comprehensive_math_functions(func_name: str, expr_str: str, val_list: list[float], expected_matches: list[float]) -> None:
    lf = pl.LazyFrame({"val": val_list})
    f = _f(expr_str, ["val"])
    # filter and check
    res = lf.filter(f.filter_expr).collect()["val"].to_list()
...
def test_complex_combined_queries() -> None:
    ...
    f1 = _f("((x / 10) + y >= 7) and y.notna() and status in ('A', 'C')", ["x", "y", "status"])
    ...
    f3 = _f("((status == 'A') or (status == 'B')) and (x * y != 100)", ["status", "x", "y"])
