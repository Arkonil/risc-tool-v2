...
class TestEditableRangeEditHandler:
    ...
    def test_valid_lower_bound_edit(self, iterations_vm, single_var_iteration):
        ...
        controls = iterations_vm._IterationsViewModel__iterations_repository.get_controls(
    ...
    def test_categorical_categories_edit(self, iterations_vm, categorical_single_var_iteration):
        ...
        cached_df.at[first_group, RangeColumn.CATEGORIES.value] = ["Employed", "Self-Employed"]
    ...
    def test_multiple_row_edits(self, iterations_vm, single_var_iteration):
        ...
        for idx, group_id in enumerate(cached_df.index):
            if group_id != RowIndex.TOTAL:
                ...
                cached_df.at[group_id, RangeColumn.UPPER_BOUND.value] = float((idx + 1) * 100)
        ...
        for idx, group_id in enumerate(controls.index):
            assert controls.at[group_id, RangeColumn.LOWER_BOUND.value] == float(idx * 100)
            assert controls.at[group_id, RangeColumn.UPPER_BOUND.value] == float((idx + 1) * 100)


class TestIterationMetricTableComponent:
    """Tests for iteration_metric_table UI component."""

    def test_single_var_iteration_metric_table_renders(self, iterations_vm, single_var_iteration):
    ...
    def test_categorical_iteration_options(self, iterations_vm, categorical_single_var_iteration):
    ...
    def test_numerical_iteration_no_categorical_options(self, iterations_vm, single_var_iteration):
