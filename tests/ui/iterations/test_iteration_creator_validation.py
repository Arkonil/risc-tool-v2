...
class TestValidateIterCreateParams:
    ...
    def test_categorical_unique_exceeds_max(self, iterations_vm):
        ...
        try:
            ...
            assert any(
                "exceeds the maximum allowed for categorical variables" in e
                for e in errors
            )
        finally:
            options_repo._OptionRepository__options_config.max_categorical_unique = (
                original_max
            )
    ...
    def test_auto_band_dlr_without_metric_variables(self, iterations_vm):
        ...
        assert any(
            "Required metric variables for DLR are not selected" in e for e in errors
        )
    ...
    def test_auto_band_ulr_without_metric_variables(self, iterations_vm):
        ...
        assert any(
            "Required metric variables for ULR are not selected" in e for e in errors
        )
    ...
    def test_auto_band_with_scalars_ulr_no_scalar_set(self, iterations_vm):
        ...
        iterations_vm._IterationsViewModel__metric_repository.var_dev_unt_bad = (
            "credit_default_flag"
        )
...
class TestIterationCreatorUIComponents:
    ...
    def test_risk_segment_details_selector_single_var(self, session):
        ...
        with (
            ...
            patch("streamlit.markdown") as _mock_markdown,
            patch("streamlit.column_config.CheckboxColumn") as _mock_checkbox_col,
            patch("streamlit.column_config.TextColumn") as _mock_text_col,
            patch("streamlit.session_state", {"session": session}),
        ):
...
class TestIterationCreatorWorkflow:
    ...
    def test_create_single_var_iteration_button_disabled_on_errors(
        self, iterations_vm, session
    ):
    ...
    def test_create_double_var_iteration_success(
    ...
    def test_validate_iter_create_params_finite_risk_segment(self, iterations_vm):
        """Test that validate_iter_create_params rejects selections without finite upper bounds."""
        errors_inf_only = iterations_vm.validate_iter_create_params(
            variable_name="credit_score",
            variable_dtype=VariableType.NUMERICAL,
            auto_band=False,
            loss_rate_type=LossRateTypes.DLR,
        ...
        assert any("finite upper bound" in err for err in errors_inf_only)

        errors_empty = iterations_vm.validate_iter_create_params(
            variable_name="credit_score",
            variable_dtype=VariableType.NUMERICAL,
            auto_band=False,
            loss_rate_type=LossRateTypes.DLR,
        ...
        assert any("finite upper bound" in err for err in errors_empty)

        errors_valid = iterations_vm.validate_iter_create_params(
            variable_name="credit_score",
            variable_dtype=VariableType.NUMERICAL,
            auto_band=False,
            loss_rate_type=LossRateTypes.DLR,
            use_scalars=False,
            selected_segment_ids=[RiskSegmentID(0), RiskSegmentID(8)],
        )
        assert not any("finite upper bound" in err for err in errors_valid)
